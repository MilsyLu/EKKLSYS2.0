...
namespace GUI
{
    public partial class FrmDetalleEventoAdmin : Form
    {
        ...
        {
            var asistentes = eventoService.ConsultarAsistentes(eventoId);
            if (asistentes.Count == 0)
            {
                MessageBox.Show("No hay asistentes registrados para este evento", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            // Mostrar lista de asistentes
            string listaAsistentes = "Lista de Asistentes:\n\n";
            foreach (var asistente in asistentes)
            {
                listaAsistentes += $"- {asistente.Nombre} {asistente.Apellido}\n";
            }

            MessageBox.Show(listaAsistentes, "Asistentes", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
